...
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum MathInstr<W> {
    Lit {val : W, reg : Reg},
    Math2 {op : M2Op, r1 : Reg, r2 : Reg, r3 : Reg},
    Jump {cond : Cond, flag : Reg, dest : Reg},
}
...
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum Instruction<U,S> {
    Signed {instr : MathInstr<S>},
    Unsigned {instr : MathInstr<U>},
...
impl<U,S> Default for Instruction<U,S> {
    fn default() -> Instruction<U,S> {
        Instruction::Invalid
    }
}

impl<U:Read,S:Read> Read for Instruction<U,S> {
    fn read<It:Iterator<Item=u8>>(it:&mut It) -> Result<Instruction<U,S>,()> {
        match it.next() {
            Some(n) => match n {
                0x0 => 
                MathInstr::<S>::read(it).and_then(|instr|
                Ok(Instruction::Signed{instr})),
                0x1 => 
                MathInstr::<U>::read(it).and_then(|instr|
                Ok(Instruction::Unsigned{instr})),
                0x2 => 
                ...
                0x05 => Ok(Instruction::Halt),
                0x06 => Ok(Instruction::Invalid),
                _ => Err(())
            },
            None => Err(())
        }  
...
impl<U : Prim + Write, S : Prim + Write> Write for Instruction<U,S> {
    fn write<Sink : WriteSink>(&self, sink: &mut Sink) -> () {
        match *self {
            Instruction::Signed{instr} => {sink.write(0x0); instr.write(sink)},
            Instruction::Unsigned{instr} => {sink.write(0x1); instr.write(sink)},
            Instruction::Ram{dir,ptr,val} => {sink.write(0x2); dir.write(sink); ptr.write(sink); val.write(sink)},
            Instruction::Out{reg} => {sink.write(0x3); reg.write(sink)},
            Instruction::Call{major,minor,arg,len} => {sink.write(0x4);major.write(sink);minor.write(sink);arg.write(sink);len.write(sink)},
            Instruction::Halt => sink.write(0x5),
            Instruction::Invalid => sink.write(0x6)
        }
    }
}
...
impl<'a, U: 'a + Compl<S>, S: 'a + Compl<U>> State<'a,U,S> {
    ...
    fn eval_instr(&mut self, instr: &Instruction<U,S>) -> Option<Result<StaticNotice<U>, Failure<U>>> {
        use self::Instruction::*;
        use self::MathInstr::*;
        // println!("{:?}", instr);
        let res = match *instr {
            Signed{instr} => match instr {
                Lit{val,reg} => {self.set_reg(reg, val.compl()); None},
                Math2{op,r1,r2,r3} => {
        ...
        let pc = match *instr {
            Unsigned {instr:Jump{cond, flag, dest}} => {
                let jump = match cond {
                    Cond::Always => true,
            ...
            Signed {instr:Jump{cond, flag, dest}} => {
                let jump = match cond {
                    Cond::Always => true,
    ...
    pub fn eval_instrs<F:Fetcher<U,Instruction<U,S>>>(&'a mut self, thrash_cnt : U, instrs : &mut F) -> Result<MutNotice<'a, U>, Failure<U>>  {
